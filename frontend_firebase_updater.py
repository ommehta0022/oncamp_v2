  ...
  Future<void> _sendOtp() async {
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          _isLoading = false;
          _errorMessage = 'Error initiating Firebase Auth: $e';
        });
      }
    }
  }
  ...
  Future<void> _signInWithCredential(PhoneAuthCredential credential) async {
    try {
      ...
      if (mounted) {
        setState(() {
          _isLoading = false;
          _errorMessage = 'Backend Verification Failed: ${e.response?.data ?? e.message}';
        });
      }
    } catch (e) {
      if (mounted) {
...
print("OTP login screen refactored for production Firebase SDK usage.")
