...
base_sql = base_sql_path.read_text(encoding="utf-8").replace('\ufeff', '')
ext_sql = ext_sql_path.read_text(encoding="utf-8").replace('\ufeff', '')

# Process ENUM types for base_sql (ext_sql already has IF NOT EXISTS for ENUMs)
types = []
def type_repl(m):
    tname = m.group(1)
    vals = m.group(2)
...
# Constraints (process on full_sql with multi-line matching)
...
full_sql = re.sub(r'ALTER\s+TABLE\s+"([^"]+)"\s+ADD\s+CONSTRAINT\s+"([^"]+)"\s+(FOREIGN\s+KEY.*?;)', constraint_repl, full_sql, flags=re.IGNORECASE | re.DOTALL)
