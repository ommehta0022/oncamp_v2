...
import { users, discoverGroups } from "@/src/data/mock";
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: colors.surface }} edges={["top"]} testID="search-screen">
      ...
      {!query ? (
        ...
        <FlatList showsVerticalScrollIndicator={false}
          data={[...discoverGroups.filter((g) => g.name.toLowerCase().includes(query.toLowerCase())), ...users.filter((u) => u.name.toLowerCase().includes(query.toLowerCase())).map((u) => ({ ...u, isUser: true }))] as any[]}
