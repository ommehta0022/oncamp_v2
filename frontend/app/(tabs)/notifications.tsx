...
export default function Notifications() {
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: colors.surface }} edges={["top"]} testID="notifications-screen">
      <View style={[styles.header, { borderBottomColor: colors.border }]}><Text style={[styles.title, { color: colors.onSurface }]}>Alerts</Text><Pressable onPress={markAll}><Text style={{ color: colors.brandPrimary, fontSize: font.base, fontWeight: "500" }}>Mark all read</Text></Pressable></View>
      <FlatList
        horizontal
        data={TABS}
        ...
        style={{ flexGrow: 0 }}
        renderItem={({ item: t }) => <Pressable onPress={() => setTab(t)} style={[styles.tab, { backgroundColor: tab === t ? colors.brandPrimary : colors.surfaceTertiary }]}><Text style={{ color: tab === t ? colors.onBrandPrimary : colors.onSurface, fontSize: font.base, fontWeight: "500" }}>{t}</Text></Pressable>}
      />
...
function NotifRow({ n, onPress }: { n: Notification; onPress: () => void }) {
  const { colors } = useTheme();
  return <Pressable onPress={onPress} style={({ pressed }) => [styles.row, { backgroundColor: !n.read ? colors.brandTertiary + "60" : pressed ? colors.surfaceTertiary : colors.surface }]}>
    <View style={{ position: "relative" }}><Avatar uri={n.avatar} name={n.title} size={44} /><View style={[styles.typeBadge, { backgroundColor: n.type === "institution_post_request" ? colors.info : colors.brandPrimary, borderColor: colors.surface }]}><Ionicons name={ICONS[n.type] || "notifications"} size={11} color="#fff" /></View></View>
    <View style={{ flex: 1 }}><Text style={{ color: colors.onSurface, fontSize: font.base, fontWeight: "500" }} numberOfLines={1}>{n.title}</Text><Text style={{ color: colors.onSurfaceTertiary, fontSize: font.sm, marginTop: 2, lineHeight: 18 }} numberOfLines={2}>{n.body}</Text><Text style={{ color: colors.muted, fontSize: 11, marginTop: 4 }}>{n.createdAt}</Text></View>
...
const styles = StyleSheet.create({
  ...
  row: { flexDirection: "row", gap: spacing.md, alignItems: "center", paddingHorizontal: spacing.lg, paddingVertical: spacing.md, minHeight: 72 },
