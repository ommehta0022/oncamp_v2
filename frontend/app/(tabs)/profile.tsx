      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 120 }}>
        <View style={{ position: "relative" }}>
          <Pressable onPress={() => cover && setViewImage(cover)}>
            {cover ? <Image source={{ uri: cover }} style={styles.cover} contentFit="cover" cachePolicy="memory-disk" /> : <View style={[styles.cover, { backgroundColor: colors.surfaceTertiary }]} />}
            {cover ? <View style={styles.coverScrim} /> : null}
          </Pressable>
          <View style={styles.topBar}><View /><Pressable onPress={() => router.push("/settings")} style={styles.iconBtn} testID="profile-settings-btn" accessibilityRole="button" accessibilityLabel="Settings"><Ionicons name="settings-outline" size={21} color="#FFFFFF" /></Pressable></View>
        </View>
        ...
        <View style={styles.profileHeader}>
          ...
          <Pressable onPress={() => router.push("/settings/edit-profile")} style={[styles.editBtn, { borderColor: colors.borderStrong }]}><Text style={{ color: colors.onSurface, fontSize: font.base, fontWeight: "700" }}>Edit profile</Text></Pressable>
        </View>

        <View style={{ paddingHorizontal: spacing.lg, marginTop: spacing.md }}>
          <View style={{ flexDirection: "row", alignItems: "center", gap: 6 }}><Text style={{ color: colors.onSurface, fontSize: 25, fontWeight: "800", letterSpacing: -0.5 }}>{user?.name || "User"}</Text>{(user as any)?.verified && <Ionicons name="checkmark-circle" size={19} color={colors.actionPrimary} />}</View>
          ...
          <View style={styles.metaWrap}>{(user as any)?.course && <Meta icon="school-outline" text={(user as any).course} />}{(user as any)?.city && <Meta icon="location-outline" text={(user as any).city} />}{streak > 0 && <Meta icon="flame-outline" text={`${streak} day streak`} color={colors.warning} />}</View>
          {loading ? <ActivityIndicator color={colors.actionPrimary} style={{ marginVertical: spacing.xl }} /> : <View style={[styles.stats, { borderColor: colors.divider }]}><Stat label="Groups" value={String(stats.groups || 0)} /><Divider /><Stat label="Day streak" value={String(streak)} highlight={streak > 0} /><Divider /><Stat label="Days here" value={String(stats.daysSinceJoin || 0)} /></View>}
        </View>

        {achievements.length > 0 && <Section title="Achievements"><ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: spacing.md }}>{achievements.map((achievement) => <View key={achievement.id} style={[styles.achievement, { backgroundColor: colors.surfaceSecondary, borderColor: colors.divider, opacity: achievement.earned === false ? .5 : 1 }]}><View style={[styles.achievementIcon, { backgroundColor: colors.selectionSoft }]}><Ionicons name={achievement.icon as any} size={22} color={achievement.earned === false ? colors.muted : colors.actionPrimary} /></View><Text style={{ color: colors.onSurface, fontSize: font.sm, fontWeight: "700", textAlign: "center", marginTop: spacing.sm }}>{achievement.label}</Text><Text style={{ color: colors.onSurfaceTertiary, fontSize: 10, textAlign: "center", marginTop: 2 }} numberOfLines={2}>{achievement.description}</Text></View>)}</ScrollView></Section>}

        <Section title="Your groups" action="See all" onAction={() => router.push("/(tabs)/groups")}>
          {groups.length === 0 ? <View style={styles.emptyBox}><Ionicons name="people-outline" size={24} color={colors.onSurfaceTertiary} /><Text style={{ color: colors.onSurfaceTertiary }}>Joined campus groups will appear here.</Text></View> : <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: spacing.md }}>{groups.map((group) => <Pressable key={group.id} onPress={() => router.push(`/group/${group.id}`)} style={[styles.groupCard, { backgroundColor: colors.surfaceSecondary, borderColor: colors.divider }]}>{group.avatarUrl || group.image ? <Image source={{ uri: group.avatarUrl || group.image }} style={styles.groupImage} contentFit="cover" cachePolicy="memory-disk" /> : <View style={[styles.groupImage, { backgroundColor: colors.surfaceTertiary, alignItems: "center", justifyContent: "center" }]}><Ionicons name="people-outline" size={28} color={colors.onSurfaceTertiary} /></View>}<View style={{ padding: spacing.md }}><Text style={{ color: colors.onSurface, fontWeight: "700" }} numberOfLines={1}>{group.name}</Text><Text style={{ color: colors.onSurfaceTertiary, fontSize: font.sm, marginTop: 2 }}>{Number(group.memberCount || group.members || 0).toLocaleString()} members</Text></View></Pressable>)}</ScrollView>}
        </Section>

        <Section title="Activity">
          <View style={[styles.workspace, { borderTopColor: colors.divider, borderBottomColor: colors.divider }]}>
...
function Section({ title, children, action, onAction }: { title: string; children: React.ReactNode; action?: string; onAction?: () => void }) { const { colors } = useTheme(); return <View style={{ marginTop: spacing.xl, paddingHorizontal: spacing.lg }}><View style={styles.sectionHeader}><Text style={{ color: colors.onSurface, fontSize: font.lg, fontWeight: "800" }}>{title}</Text>{action && onAction && <Pressable onPress={onAction}><Text style={{ color: colors.link, fontSize: font.base, fontWeight: "800" }}>{action}</Text></Pressable>}</View>{children}</View>; }
...
function Stat({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) { const { colors } = useTheme(); return <View style={{ flex: 1, alignItems: "center" }}><Text style={{ color: highlight ? colors.warning : colors.onSurface, fontSize: font.xl, fontWeight: "800" }}>{value}</Text><Text style={{ color: colors.onSurfaceTertiary, fontSize: font.sm, marginTop: 2 }}>{label}</Text></View>; }
function Divider() { const { colors } = useTheme(); return <View style={{ width: StyleSheet.hairlineWidth, height: 30, backgroundColor: colors.divider }} />; }
function Action({ icon, label, onPress }: { icon: any; label: string; onPress: () => void }) { const { colors } = useTheme(); return <Pressable onPress={onPress} style={[styles.action, { borderBottomColor: colors.divider }]}><Ionicons name={icon} size={18} color={colors.onSurfaceTertiary} /><Text style={{ flex: 1, color: colors.onSurface, fontSize: font.base, fontWeight: "600" }}>{label}</Text><Ionicons name="chevron-forward" size={17} color={colors.onSurfaceTertiary} /></Pressable>; }

const styles = StyleSheet.create({
  cover: { width: "100%", height: 172 },
  coverScrim: { position: "absolute", left: 0, right: 0, top: 0, height: 172, backgroundColor: "rgba(0,0,0,0.18)" },
  topBar: { position: "absolute", top: 0, left: 0, right: 0, flexDirection: "row", justifyContent: "space-between", padding: spacing.md },
  iconBtn: { width: 40, height: 40, borderRadius: 20, alignItems: "center", justifyContent: "center", backgroundColor: "rgba(0,0,0,0.48)" },
  profileHeader: { flexDirection: "row", alignItems: "flex-start", justifyContent: "space-between", paddingHorizontal: spacing.lg },
  editBtn: { marginTop: spacing.md, paddingHorizontal: spacing.lg, height: 40, borderRadius: radius.pill, borderWidth: StyleSheet.hairlineWidth, alignItems: "center", justifyContent: "center" },
  ...
  achievement: { width: 126, borderWidth: StyleSheet.hairlineWidth, borderRadius: radius.md, padding: spacing.md, alignItems: "center" },
  achievementIcon: { width: 42, height: 42, borderRadius: 21, alignItems: "center", justifyContent: "center" },
  groupCard: { width: 174, borderWidth: StyleSheet.hairlineWidth, borderRadius: radius.md, overflow: "hidden" },
  groupImage: { width: "100%", height: 88 },
  emptyBox: { paddingVertical: spacing.xl, alignItems: "center", gap: spacing.sm },
  workspace: { borderTopWidth: StyleSheet.hairlineWidth, borderBottomWidth: StyleSheet.hairlineWidth },
