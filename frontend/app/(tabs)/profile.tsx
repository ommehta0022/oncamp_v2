...
function Section({ title, children, action, onAction }: { title: string; children: React.ReactNode; action?: string; onAction?: () => void }) { const { colors } = useTheme(); return <View style={{ marginTop: spacing.xl, paddingHorizontal: spacing.lg }}><View style={styles.sectionHeader}><Text style={{ color: colors.onSurface, fontSize: font.lg, fontWeight: "800" }}>{title}</Text>{action && onAction && <Pressable onPress={onAction}><Text style={{ color: colors.luxuryGold, fontSize: font.base, fontWeight: "800" }}>{action}</Text></Pressable>}</View>{children}</View>; }
...
function Stat({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) { const { colors } = useTheme(); return <View style={{ flex: 1, alignItems: "center" }}><Text style={{ color: highlight ? colors.luxuryGold : colors.onSurface, fontSize: font.xl, fontWeight: "800" }}>{value}</Text><Text style={{ color: colors.onSurfaceTertiary, fontSize: font.sm, marginTop: 2 }}>{label}</Text></View>; }
